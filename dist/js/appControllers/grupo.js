...
(function () {
    ...
    function GridInterno() {
        ...
        function getGrid(vm, columnsDefs, $scope, funct) {
            ...
            vm.filaSelecciona = [];
            vm.gridDemo.appScopeProvider = vm; 
            vm.getPage = function () {
                vm.filter['pageSize'] = vm.gridDemo.paginationPageSize;
    ...
    modalGrupoModuloInstanceCtrl.$inject = ['$scope', '$state', '$uibModalInstance', 'modalParam', 'grupoService', 'rolService', 'GridInterno', 'uiGridConstants', 'Notification', '$timeout', '$log'];
    function modalGrupoModuloInstanceCtrl($scope, $state, $uibModalInstance, modalParam, grupoService, rolService, GridInterno, uiGridConstants, Notification, $timeout, $log) {
        var vm = this;
        ...
        function loadModulos() {
            ...
            var accion = '<a class="btn btn-primary btn-xs" ng-click="grid.appScope.deleteRow(row);"> Asignar</a>';
            ...
            GridInterno.getGrid(vm, columnsDefs, $scope, function () {
                rolService.GetIndex(vm.filter).then(function (grupos) {
                
                    vm.gridDemo.totalItems = grupos.total;
                    vm.gridDemo.data = grupos.data;
                    $timeout(function () {
                        if (vm.gridApi.selection.selectRow) {
                            vm.gridApi.selection.selectRow(vm.gridDemo.data[0]);
                        }
                    });
                });
            });
