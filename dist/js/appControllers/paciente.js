"use strict";
(function () {
    angular.module('adminLTEApp.paciente', [])
            .controller('listPacientelCtrl', listPacientelCtrl)
            .controller('newPacientelCtrl', newPacientelCtrl)
            .controller('editPacientelCtrl', editPacientelCtrl)
            .controller('modalPacientelCtrl', modalPacientelInstanceCtrl)
            .controller('modalPacientelEyeInstanceCtrl', modalPacientelEyeInstanceCtrl);

    listPacientelCtrl.$inject = ['$scope', '$state', 'pacienteService', 'GridExternal', 'uiGridConstants' ,'$uibModal', '$timeout', '$log' ];
    function listPacientelCtrl($scope, $state, pacienteService, GridExternal, uiGridConstants, $uibModal, $timeout, $log ) {
        var vm = this;
        ...
        function loadPacientes() {
            var columnsDefs = [
                {name: 'Id', field: 'idPacientel', width: '42'},
            ...
            GridExternal.getGrid(vm, columnsDefs, $scope, function () {
                pacienteService.GetIndex(vm.filter).then(function (pacientes) {

                    vm.gridOptions.totalItems = pacientes.total;
                    vm.gridOptions.data = pacientes.data;
                    $timeout(function () {
                        if (vm.gridApi.selection.selectRow) {
                            vm.gridApi.selection.selectRow(vm.gridOptions.data[0]);
                        }
                    });
                    GridExternal.setInfoPagina(vm, vm.gridApi.pagination, vm.gridOptions, pacientes.data.length);
                });
            });
        }
    ...
    newPacientelCtrl.$inject = ['$scope', '$state', 'pacienteService', 'ubigeoService','GridInternoTurno', 'Notification'];
    function newPacientelCtrl($scope, $state, pacienteService, ubigeoService, GridInternoTurno, Notification) {
        var vm = this;
        ...
        switch (vm.stateparent) {
            case 'Pacientel':
                vm.descripcion = 'Registrarlo como Pacientel';
        ...
        vm.save = function () {
            ...
            vm.OBJentidad = {
                ...
                tipoentidad: vm.stateparent// Tablas: cliente, proveedor, medico, Pacientel 
            };
    ...
    editPacientelCtrl.$inject = ['$scope', '$state', 'pacienteService', 'ubigeoService', 'contactoService', 'GridInternoTurno', '$stateParams', 'GridExternal', 'Notification', '$uibModal', '$log'];
    function editPacientelCtrl($scope, $state, pacienteService, ubigeoService, contactoService, GridInternoTurno ,$stateParams, GridExternal, Notification, $uibModal, $log) {
        var vm = this;
        ...
        entidadService.GetShow($stateParams.entidadId, {tipoentidad: vm.stateparent}).then(function (entidades) {
            ...
            vm.Pacientel = entidades.others.Pacienteles;
    ...
    modalPacientelInstanceCtrl.$inject = ['$scope', '$uibModalInstance', 'modalParam', 'entidadService', 'Notification'];
    function modalPacientelInstanceCtrl($scope, $uibModalInstance, modalParam, entidadService, Notification) {
    ...
    modalPacientelEyeInstanceCtrl.$inject = ['$scope', '$uibModalInstance', 'modalParam', 'pacienteService', '$state'];
    function modalPacientelEyeInstanceCtrl($scope, $uibModalInstance, modalParam, pacienteService, $state) {
        ...
        pacienteService.GetProfile(modalParam.data).then(function (entidades) {
            vm.entidad = entidades.data;
            vm.others = entidades.others;
            vm.cliente = entidades.others.clientes;
            vm.Pacientel = entidades.others.Pacienteles;
