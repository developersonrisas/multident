(function () {
    ...
    angular
            .module('adminLTE')
            .factory('odontologoService', odontologoService);

    odontologoService.$inject = ['$http', '$rootScope'];     
    function odontologoService($http, $rootScope) {
        var service = {}; 
        ...
        function GetIndex(request) {  
            return $http.get($rootScope.api + '/odontologo', {params: request}).then(handleSuccess, handleError('Error al consultar rol'));             
        }
        
        //Autocompletable
        function GetSearch(request) {     // le mando el likepersonal 
            return $http.get($rootScope.servidor + '/odontologo', {params: request});
        }
        
        //Visualizar un registro
        function GetShow(id, request){ 
            return $http.get($rootScope.servidor + '/odontologo/' + id, {params: request}).then(handleSuccess, handleError('Error getting all producto'));
        }
