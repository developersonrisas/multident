(function () {
    ...
    pacienteService.$inject = ['$http', '$rootScope'];     
    function pacienteService($http, $rootScope) {
        var service = {}; 
        ...
        function GetIndex(request) {  
            return $http.get($rootScope.api + '/paciente', {params: request}).then(handleSuccess, handleError('Error al consultar rol'));             
        }
        
        //Autocompletable
        function GetSearch(request) {     // le mando el likepersonal 
            return $http.get($rootScope.servidor + '/paciente', {params: request});
        }
        
        //Visualizar un registro
        function GetShow(id, request){ 
            return $http.get($rootScope.servidor + '/paciente/' + id, {params: request}).then(handleSuccess, handleError('Error getting all producto'));
        }
